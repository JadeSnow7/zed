...
const LEGACY_MISSION_MIGRATION: &str = sql!(
...
/// Opens a persistent metadata database without the normal in-memory fallback
/// and upgrades the one known pre-split Mission schema in place. The backup is
/// made before changing the migration ledger, so a failed upgrade never
/// destroys the only copy of the user's data.
pub fn preflight_legacy_mission_database(path: &Path) -> anyhow::Result<()> {
    if !path.exists() {
        return Ok(());
    }

    let connection =
        db::sqlez::connection::Connection::open(path.to_string_lossy().as_ref(), true)?;
    connection.exec("PRAGMA busy_timeout = 500")?()?;
    let tables = connection
        .select::<(String, Option<String>)>(
            "SELECT name, sql FROM sqlite_master WHERE type = 'table' AND name IN ('missions', 'sidebar_threads')",
        )?()?
        .into_iter()
        .collect::<HashMap<_, _>>();

    let mut migrations_table_query = connection.select_row::<i64>(
        "SELECT COUNT(*) FROM sqlite_master WHERE type = 'table' AND name = 'migrations'",
    )?;
    let has_migrations_table = migrations_table_query()?.unwrap_or_default() > 0;
    if !has_migrations_table {
        if tables.is_empty() {
            return Ok(());
        }
        bail!("metadata database has schema without a migrations ledger");
    }

    let mut migration_query = connection.select::<(String, usize, String)>(
        "SELECT domain, step, migration FROM migrations ORDER BY domain, step",
    )?;
    let migration_rows = migration_query()?;
    let thread_rows: Vec<_> = migration_rows
        .iter()
        .filter(|(domain, _, _)| domain == ThreadMetadataDb::NAME)
        .collect();
    let mission_rows: Vec<_> = migration_rows
        .iter()
        .filter(|(domain, _, _)| domain == MissionDb::NAME)
        .collect();

    for (_, step, migration) in &thread_rows {
        if *step < ThreadMetadataDb::MIGRATIONS.len()
            && *migration != ThreadMetadataDb::MIGRATIONS[*step]
        {
            bail!("ThreadMetadataDb migration ledger does not match the installed schema");
        }
        if *step == ThreadMetadataDb::MIGRATIONS.len() && *migration != LEGACY_MISSION_MIGRATION {
            bail!(
                "unknown ThreadMetadataDb Mission migration in {}",
                path.display()
            );
        }
        if *step > ThreadMetadataDb::MIGRATIONS.len() {
            bail!("unknown ThreadMetadataDb migration step {step}");
        }
    }
    for (_, step, migration) in &mission_rows {
        if *step != 0 || *migration != mission_db::MissionDb::MIGRATIONS[0] {
            bail!("MissionDb migration ledger does not match the installed schema");
        }
    }
    if mission_rows.len() > 1 {
        bail!("duplicate MissionDb migration ledger entries");
    }

    if tables.contains_key("sidebar_threads") {
        for expected_step in 0..ThreadMetadataDb::MIGRATIONS.len() {
            if thread_rows
                .iter()
                .filter(|(_, step, _)| *step == expected_step)
                .count()
                != 1
            {
                bail!("ThreadMetadataDb migration ledger is incomplete or duplicated");
            }
        }
    }

    let sidebar_columns: HashSet<String> =
        connection.select::<String>("SELECT name FROM pragma_table_info('sidebar_threads')")?()?
            .into_iter()
            .collect();
    let has_mission_columns =
        sidebar_columns.contains("mission_id") && sidebar_columns.contains("role");
    let has_partial_mission_columns =
        sidebar_columns.contains("mission_id") || sidebar_columns.contains("role");
    let has_missions_table = tables.contains_key("missions");
    let legacy = thread_rows.iter().any(|(_, step, migration)| {
        *step == ThreadMetadataDb::MIGRATIONS.len() && *migration == LEGACY_MISSION_MIGRATION
    });
    let mission_ledger = !mission_rows.is_empty();

    if mission_ledger {
        if !has_missions_table || !has_mission_columns || legacy {
            bail!("MissionDb ledger conflicts with the installed Mission schema");
        }
        return Ok(());
    }

    if legacy {
        if !has_missions_table
            || !has_mission_columns
            || thread_rows.len() != ThreadMetadataDb::MIGRATIONS.len() + 1
        {
            bail!("legacy Mission migration ledger conflicts with the installed schema");
        }
        let backup_path = path.with_extension(format!(
            "mission-migration-backup-{}.sqlite",
            uuid::Uuid::new_v4()
        ));
        std::fs::OpenOptions::new()
            ...
            .open(&backup_path)?;
        let backup =
            db::sqlez::connection::Connection::open(backup_path.to_string_lossy().as_ref(), true)?;
        connection.backup_main(&backup)?;
        connection.with_savepoint("mission_ledger_upgrade", || {
            connection.exec_bound("DELETE FROM migrations WHERE domain = ? AND step = ?")?((
                ThreadMetadataDb::NAME,
                ThreadMetadataDb::MIGRATIONS.len(),
            ))?;
            connection
                .exec_bound("INSERT INTO migrations (domain, step, migration) VALUES (?, ?, ?)")?(
                ("MissionDb", 0usize, mission_db::MissionDb::MIGRATIONS[0]),
            )?;
            Ok(())
        })?;
        return Ok(());
    ...
    if has_partial_mission_columns || has_missions_table {
        bail!(
            "metadata database has a partial Mission migration: {}",
            path.display()
        );
    }

    if !tables.contains_key("sidebar_threads") && !thread_rows.is_empty() {
        bail!("ThreadMetadataDb ledger exists without sidebar_threads");
    }
...
mod tests {
    ...
    fn preflight_fixture_path(label: &str) -> std::path::PathBuf {
        std::env::temp_dir().join(format!(
            "zed-mission-preflight-{label}-{}.sqlite",
            uuid::Uuid::new_v4()
        ))
    }

    fn remove_preflight_fixture(path: &std::path::Path) {
    ...
    fn create_legacy_preflight_fixture(
        path: &std::path::Path,
    ) -> db::sqlez::connection::Connection {
        use db::sqlez::connection::Connection;

        let connection = Connection::open(path.to_string_lossy().as_ref(), true).unwrap();
        connection
            .migrate(
                ThreadMetadataDb::NAME,
                ThreadMetadataDb::MIGRATIONS,
            ...
            .unwrap();
        connection.exec(LEGACY_MISSION_MIGRATION).unwrap()().unwrap();
        connection
            .exec_bound("INSERT INTO migrations (domain, step, migration) VALUES (?, ?, ?)")
            .unwrap()((
            ThreadMetadataDb::NAME,
            ThreadMetadataDb::MIGRATIONS.len(),
            LEGACY_MISSION_MIGRATION,
        ))
        .unwrap();
        connection
            .exec(
                "INSERT INTO sidebar_threads (thread_id, session_id, title, updated_at, mission_id, role) \
                 VALUES (X'0102030405060708090A0B0C0D0E0F10', 'legacy', 'Legacy', '2025-01-01T00:00:00Z', NULL, NULL)",
            )
            .unwrap()()
            .unwrap();
        connection
    ...
    #[test]
    fn preflight_migrates_legacy_database_without_losing_rows_and_is_idempotent() {
        use db::sqlez::connection::Connection;

        let path = preflight_fixture_path("legacy");
        let connection = create_legacy_preflight_fixture(&path);
        drop(connection);

        preflight_legacy_mission_database(&path).unwrap();
        preflight_legacy_mission_database(&path).unwrap();

        let connection = Connection::open(path.to_string_lossy().as_ref(), true).unwrap();
        let ledger: Vec<(String, usize, String)> = connection
            .select("SELECT domain, step, migration FROM migrations ORDER BY domain, step")
            .unwrap()()
        .unwrap();
        assert!(!ledger.iter().any(|(domain, step, _)| {
            domain == ThreadMetadataDb::NAME && *step == ThreadMetadataDb::MIGRATIONS.len()
        }));
        assert_eq!(
            ledger
                .iter()
                .filter(|(domain, step, _)| domain == MissionDb::NAME && *step == 0)
                .count(),
            1
        );
        assert_eq!(
            ledger
                .iter()
                .find(|(domain, step, _)| domain == MissionDb::NAME && *step == 0)
                .map(|(_, _, migration)| migration.as_str()),
            Some(MissionDb::MIGRATIONS[0])
        );
        let title: String = connection
            .select_row("SELECT title FROM sidebar_threads WHERE session_id = 'legacy'")
            .unwrap()()
        .unwrap()
        .unwrap();
        assert_eq!(title, "Legacy");
        remove_preflight_fixture(&path);
    }

    #[test]
    fn preflight_accepts_fresh_database_and_rejects_unknown_legacy_step() {
        use db::sqlez::connection::Connection;

        let fresh_path = preflight_fixture_path("fresh");
        let connection = Connection::open(fresh_path.to_string_lossy().as_ref(), true).unwrap();
        connection
            .migrate(
                ThreadMetadataDb::NAME,
                ThreadMetadataDb::MIGRATIONS,
            ...
            .unwrap();
        drop(connection);
        preflight_legacy_mission_database(&fresh_path).unwrap();
        remove_preflight_fixture(&fresh_path);

        let unknown_path = preflight_fixture_path("unknown");
        let connection = Connection::open(unknown_path.to_string_lossy().as_ref(), true).unwrap();
        connection
            .migrate(
                ThreadMetadataDb::NAME,
                ThreadMetadataDb::MIGRATIONS,
        ...
        .unwrap();
        drop(connection);
        assert!(preflight_legacy_mission_database(&unknown_path).is_err());
        remove_preflight_fixture(&unknown_path);
    }

    #[test]
    fn preflight_reports_locked_database_instead_of_falling_back() {
        let path = preflight_fixture_path("locked");
        let connection = create_legacy_preflight_fixture(&path);
        connection.exec("BEGIN EXCLUSIVE").unwrap()().unwrap();
        let result = preflight_legacy_mission_database(&path);
        connection.exec("ROLLBACK").unwrap()().unwrap();
        assert!(result.is_err());
        drop(connection);
        remove_preflight_fixture(&path);
    }
